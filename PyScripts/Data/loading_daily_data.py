...
#!/usr/bin/env python3
# Example:
#   YF_NETWORK_PROFILE=stable .venv/bin/python PyScripts/Data/loading_daily_data.py
...
for _ in range(10):
    if ROOT.name == "STAT-587-Final-Project":
        break
    ROOT = ROOT.parent
else:
    raise FileNotFoundError("Could not find correct workspace folder.")

DATA_DIR = ROOT / "PyScripts" / "Data"
TICKERS_FILE = DATA_DIR / "tickers.csv"
LOOKUP_FILE = DATA_DIR / "stock_lookup_table.csv"
OUTPUT_DIR = DATA_DIR
...
# Keep False unless you explicitly want yfinance repair mode.
...
NETWORK_PROFILE = os.getenv("YF_NETWORK_PROFILE", "stable").strip().lower()
...
def _ensure_ticker_first_columns(dataframe: pd.DataFrame, tickers: list[str]) -> pd.DataFrame:
    """Ensure DataFrame columns are MultiIndex with ticker at level 0."""
    if dataframe.empty:
        return dataframe
...
def _load_stock_tickers() -> list[str]:
    tickers = pd.read_csv(TICKERS_FILE, usecols=["Ticker"])["Ticker"].dropna().astype(str).str.strip()
    lookup_tickers = pd.read_csv(LOOKUP_FILE, usecols=["Ticker"])["Ticker"].dropna().astype(str).str.strip()
...
def _download_with_retries(
    ...
    """Download one batch with retry logic."""
    last_error = None
...
def _extract_ticker_frame(raw: pd.DataFrame, ticker: str, asset_type: str) -> pd.DataFrame | None:
    """Return one ticker's normalized frame, or None if incomplete."""
    if ticker not in raw.columns.get_level_values(0):
        return None

    ticker_df = raw[ticker].copy()

    if not all(field in ticker_df.columns for field in PRICE_FIELDS):
        return None

    ticker_df = ticker_df.loc[:, PRICE_FIELDS].copy()
...
def _download_type_block(tickers: list[str], asset_type: str) -> pd.DataFrame:
    """
    Download one asset class in batches.

    Optimizations:
    - while-loop so fallback batch_size changes actually take effect
    - store ticker frames directly; concat only once at the end
    - single-ticker recovery only for a small unresolved set
    """
    ...
    while start < len(tickers):
        ...
        if batch_frames:
            pieces.extend(batch_frames)

        start += len(batch)
        batch_num += 1

    # Recovery pass: retry unresolved tickers in smaller batches
    ...
    for i in range(0, len(recovery_candidates), FALLBACK_BATCH_SIZE):
        batch = recovery_candidates[i:i + FALLBACK_BATCH_SIZE]
        recovery_num = i // FALLBACK_BATCH_SIZE + 1

        raw = _download_with_retries(
            batch,
            asset_type=asset_type,
            batch_label=f"recovery batch {recovery_num}",
            retry_attempts=retry_attempts,
            retry_wait_seconds=retry_wait_seconds,
            threads=False,
        )
    ...
    # Final pass: only recover individually when the unresolved set is small enough
    final_unresolved = unresolved
    single_recovery_cap = min(MAX_SINGLE_TICKER_RECOVERY, 15)

    if unresolved and len(unresolved) <= single_recovery_cap:
        final_unresolved = []
...
def main() -> None:
    ...
    for asset_type, tickers in asset_map.items():
        ...
        print(f"{asset_type} download time: {time.time() - asset_t0:.2f} seconds")
        if not block.empty:
    ...
    if FORCE_CALENDAR_START:
        full_index = pd.date_range(start=START_DATE, end=daily.index.max(), freq="D")
    ...
    print(f"Total runtime: {time.time() - total_t0:.2f} seconds")
