#!/usr/bin/env python3
# Example:
#   YF_NETWORK_PROFILE=stable .venv/bin/python PyScripts/Data/loading_hourly_data.py
...
OUTPUT_DIR = DATA_DIR
OUTPUT_FILE = OUTPUT_DIR / "hourly_data.parquet"
...
# Match daily loader behavior: adjusted OHLC prices.
...
def _extract_ticker_frame(raw: pd.DataFrame, ticker: str, asset_type: str) -> pd.DataFrame | None:
    ...
    # Match daily loader: require the full OHLCV field set.
...
def main() -> None:
    ...
    hourly = pd.concat(type_frames, axis=1)
    ...
    hourly.columns = pd.MultiIndex.from_tuples(hourly.columns, names=["Metric", "Type", "Ticker"])
    ...
    print("Column levels:", hourly.columns.names)
    print("Metric values:", list(hourly.columns.get_level_values(0).unique()))
    print("Type values:", list(hourly.columns.get_level_values(1).unique()))
