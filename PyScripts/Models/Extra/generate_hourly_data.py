...
REFERENCE_FILE = cwd / "PyScripts" / "raw_data.parquet"
...
def main() -> None:
    reference = pq.read_table(REFERENCE_FILE).to_pandas()
    columns = reference.columns

    all_types = sorted(columns.get_level_values(1).unique())
    print(f"Types found in reference: {all_types}")

    type_frames = []
    for asset_type in all_types:
        tickers = sorted(columns[columns.get_level_values(1) == asset_type].get_level_values(2).unique())
