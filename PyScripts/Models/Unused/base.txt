...
if __name__ == "__main__":
    ...
    for n_comp in n_components_grid:
        ...
        if mean_score > best_score:
            ...
            best_n_comp = n_comp
            best_n_comp_value = X_pca_temp.shape[1]
    
    print(f"\nBest n_components: {best_n_comp} ({best_n_comp_value} components) with CV Score = {best_score:.4f}")
    ...
    rows = [
        ...
        _metrics('Ridge CV (raw)',    pipeline_ridge, X_train, y_train, X_test, y_test, tscv, best_c=ridge_cv.C_[0]),
        _metrics('LASSO CV (raw)',    pipeline_lasso, X_train, y_train, X_test, y_test, tscv, best_c=lasso_cv.C_[0]),
        _metrics(f'Ridge CV (PCA-{n_components_raw})',    clf_ridge_pca,  X_train_pca, y_train, X_test_pca, y_test, tscv, best_c=clf_ridge_pca.C_[0]),
        _metrics(f'LASSO CV (PCA-{n_components_raw})',    clf_lasso_pca,  X_train_pca, y_train, X_test_pca, y_test, tscv, best_c=clf_lasso_pca.C_[0]),
    ]
    ...
    n_components_grid = [0.85, 0.90, 0.95, 0.99]
    best_score_dow = -np.inf
    best_n_comp_dow = None
    best_n_comp_value_dow = None
    ...
    for n_comp in n_components_grid:
        ...
        if mean_score > best_score_dow:
            best_score_dow = mean_score
            best_n_comp_dow = n_comp
            best_n_comp_value_dow = X_pca_temp.shape[1]
    
    print(f"\nBest n_components: {best_n_comp_dow} ({best_n_comp_value_dow} components) with CV Score = {best_score_dow:.4f}")
    ...
    rows_dow = [
        ...
        _metrics('Ridge CV+DOW',       pipeline_ridge_dow, X_train_dow, y_train, X_test_dow, y_test, tscv, best_c=pipeline_ridge_dow.named_steps['classifier'].C_[0]),
        _metrics('LASSO CV+DOW',       pipeline_lasso_dow, X_train_dow, y_train, X_test_dow, y_test, tscv, best_c=pipeline_lasso_dow.named_steps['classifier'].C_[0]),
        _metrics(f'Ridge CV+PCA ({n_components_dow})+DOW',   clf_ridge_pca_dow,  X_train_dow_pca, y_train, X_test_dow_pca, y_test, tscv, best_c=clf_ridge_pca_dow.C_[0]),
        _metrics(f'LASSO CV+PCA ({n_components_dow})+DOW',   clf_lasso_pca_dow,  X_train_dow_pca, y_train, X_test_dow_pca, y_test, tscv, best_c=clf_lasso_pca_dow.C_[0]),
    ]
